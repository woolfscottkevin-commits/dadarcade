...
- **Relative asset paths 404 on production.** Pages are served at `/<slug>` with **no trailing slash** (`cleanUrls` + `trailingSlash:false`), so a bare `assets/x.png` resolves to the domain root (`/assets/x.png`) and 404s — and a game with graceful fallbacks then silently shows placeholders with no error. Always add `<base href="/<slug>/">` in `<head>` for any game that loads assets by relative path (see `balloon-uncle-bop`, `dad-quest`, `par-3`). Verify against the **no-trailing-slash** URL — testing `/<slug>/` with a slash hides the bug.
