...
ok(isUsableStory(story("Thieves stole Renoir paintings from a French museum"), today), "an art heist is allowed — nobody is hurt and a nine-year-old can have a heist story");
ok(!isUsableStory(story("Gunman shot a guard during a museum robbery"), today), "…but a robbery where someone was hurt is still blocked, by the violence list");
ok(!isUsableStory(story("Poachers target rhino in reserve"), today), "harm to animals stays off the show");
ok(!isUsableStory(story("Thousands of birds culled after outbreak"), today), "so does a cull");
